...
InferenceRunner::InferenceRunner(const std::string& engineFilePath, Logger& useLogger):
    m_logger(useLogger),
    m_runtime(
...
void InferenceRunner::createNewEngine(const std::string& engineFilePath)
{
...
void InferenceRunner::createNewExecutionContext()
{
    if(!m_engine)
