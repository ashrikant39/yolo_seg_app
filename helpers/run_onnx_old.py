...
def decode_nms_masks(nms_pred, nms_proto, img_w, img_h, conf_thres=0.25, mask_thres=0.5):
    """
    ...
    print(f"nms_pred shape : {pred.shape} : Type : {pred.dtype}")
    print(f"nms_proto shape: {proto.shape} : Type : {proto.dtype}")
    ...
    if len(boxes) == 0:
        return boxes, scores, classes, np.empty((0, proto.shape[1], proto.shape[2])), np.empty((0, proto.shape[1], proto.shape[2]), dtype=bool)
    ...
    # proto:  [nm, mh, mw]
    # -> mask_logits: [K, mh, mw]
    mask_logits = coeffs @ proto.reshape(nm, -1)
    ...
    # crop in proto space
    mask_logits = crop_mask_np(mask_logits, boxes_proto)

    # sigmoid -> probabilities
    mask_probs = sigmoid(mask_logits)

    # binary masks
    mask_bin = mask_probs > mask_thres

    return boxes, scores, classes, mask_probs, mask_bin


def main():
    os.makedirs("masks/no_nms_bin", exist_ok=True)
    os.makedirs("masks/no_nms_prob", exist_ok=True)
    os.makedirs("masks/no_nms_overlay", exist_ok=True)
    ...
    onnx_nms_path = "assets/onnx/last_bs1_nms.onnx"
    ...
    #     plt.imsave(f"masks/no_nms_prob/mask_prob_{i:03d}.png", prob_up, cmap="gray")

    #     # Save binary mask
    #     plt.imsave(f"masks/no_nms_bin/mask_bin_{i:03d}.png", bin_up.astype(np.float32), cmap="gray")
    ...
    #         cv2.LINE_AA,
    #     )
    #     cv2.imwrite(f"masks/no_nms_overlay/overlay_{i:03d}.png", overlay_bgr)
